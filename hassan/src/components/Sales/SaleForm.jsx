...
// import api from '../../services/api.js';
// import Cart from './Cart.jsx';
// import {  FaArrowLeft, FaCheck, FaTimes, FaPlus, FaUser, FaShoppingBag, FaSearch,FaBox, FaInfoCircle} from 'react-icons/fa';
// import {  Container, Row, Col, Form, Button, Card, Table,  InputGroup, Badge, Alert, Spinner, Modal, Toast} from 'react-bootstrap';
...
//           setError('Erreur lors du chargement des  vente ');
//         } finally {
//           setLoading(false);
//         }
//       };
...
//   client_name: sale.client_name,
//   packaging_included: sale.packaging_included,
//   packaging_count: sale.packaging_count, // ← important
//   notes: sale.notes,
//   items
// };
...
//         if (id) {
//            // Mise à jour des items de la vente
//              await api.put(`/sales/${id}/items`, {
//              items: cart.map((item) => ({
//               product_id: item.product_id,
//               quantity: item.quantity
...
//   // Rechargement des données de la vente depuis le backend
//          const response = await api.get(`/sales/${id}`);
  
//            setSale(response.data.sale);
//           setCart(response.data.items.map(item => ({
//          ...item,
//              maxQuantity: item.product_stock + item.quantity
//             })));
...
import Cart from './Cart.jsx';
import {  
  FaArrowLeft, FaCheck, FaTimes, FaPlus, FaUser, FaShoppingBag, FaSearch, FaBox, FaInfoCircle
} from 'react-icons/fa';
import {  
  Container, Row, Col, Form, Button, Card, Table, InputGroup, Badge, Alert, Spinner, Modal, Toast
} from 'react-bootstrap';
...
const SaleForm = () => {
  ...
  // Charger produits en tenant compte du stock
  useEffect(() => {
    if (products.length) {
      setAvailableProducts(products.map(p => ({
        ...p,
  ...
  // Charger vente existante si id présent
  useEffect(() => {
    if (id) {
      const fetchSale = async () => {
        setLoading(true);
        try {
          ...
          // Set cart items avec maxQuantity pour modification
          setCart(response.data.items.map(item => ({
            ...item,
            maxQuantity: item.product_stock + item.quantity
          })));

          // Set packagingCount si existant
          if (response.data.sale.packaging_count) {
            setPackagingCount(response.data.sale.packaging_count);
          }
        } catch (error) {
  ...
  // Mise à jour des champs de la vente
  const handleSaleChange = (e) => {
    const { name, value, type, checked } = e.target;
    setSale({
      ...sale,
    ...
    // Si décoché emballage, reset packagingCount
    if (name === 'packaging_included' && !checked) {
      setPackagingCount(0);
    }
  };
  ...
  const addToCart = (product) => {
    ...
    if (existingItem) {
      ...
      setCart(cart.map(item =>
        item.product_id === product.id
          ? { ...item, quantity: item.quantity + 1, maxQuantity: product.stock }
          : item
      ));
    } else {
      if (product.stock < 1) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const saleData = {
        client_name: sale.client_name,
        packaging_included: sale.packaging_included,
        packaging_count: sale.packaging_included ? packagingCount : 0, // Important!
        notes: sale.notes,
        items
      };
      ...
      if (id) {
        ...
        setPackagingCount(response.data.sale.packaging_count || 0);
        setSuccess('Vente mise à jour avec succès');
      } else {
        // Création
        await api.post('/sales', saleData);
  ...
  // Confirmation / Annulation simplifiée...
  const confirmSale = async () => {
    setLoading(true);
    try {
      await api.put(`/sales/${id}/confirm`);
  ...
  return (
    <Container className="py-4">
        ...
        <Row>
              ...
              <Table hover bordered size="sm" className="mb-0">
                ...
                <tbody>
                  {filteredProducts.length === 0 ? (
                    ...
                    filteredProducts.map(product => (
                      <tr key={product.id} className={product.disabled ? 'table-secondary' : ''}>
                        <td>{product.name}</td>
                        <td>{product.category}</td>
                        <td className="text-end">{product.price.toLocaleString('fr-FR')} FCFA</td>
                        ...
                        <td className="text-center">
                          ...
                          {stockErrors[product.id] && (
                            <div className="text-danger small mt-1">{stockErrors[product.id]}</div>
                          )}
                        </td>
                      </tr>
                    ))
          ...
          <Col lg={4}>
            <Cart
              ...
              isEdit={!!id}
              packagingIncluded={sale.packaging_included}
              packagingPrice={5_000} // Exemple, adapte selon ta logique
              onPackagingChange={setPackagingCount} // <-- important !
            />
          </Col>
        </Row>
      </Form>
      ...
      <Modal show={showConfirmModal} onHide={() => setShowConfirmModal(false)} centered>
        ...
        <Modal.Body>
          <p>Voulez-vous vraiment confirmer cette vente ?</p>
        </Modal.Body>
        <Modal.Footer>
