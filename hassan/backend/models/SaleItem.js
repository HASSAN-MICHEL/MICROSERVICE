...
//   static async updatesale(sale_id, product_id, quantity) {
//   const query = `
//     UPDATE sale_items
//     SET quantity = $3
...
//     RETURNING *;
//   `;
//   const values = [sale_id, product_id, quantity];
//   const { rows } = await pool.query(query, values);
//   return rows[0];
// }
...
// static async deleteItem(sale_id, product_id) {
//   const query = `DELETE FROM sale_items WHERE sale_id = $1 AND product_id = $2`;
//   await pool.query(query, [sale_id, product_id]);
// }
...
class SaleItem {
  ...
  static async updateQuantity(sale_id, product_id, quantity) {
    const query = `
      UPDATE sale_items
      SET quantity = $3
      WHERE sale_id = $1 AND product_id = $2
  ...
  static async getDailySalesReport(date) {
    ...
    try {
      ...
      console.error('Erreur lors du rapport journalier:', error);
      throw new Error('Erreur base de données');
    }
  }
